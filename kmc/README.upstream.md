...
#### Per-strand counters for canonical k-mers
By default a canonical count merges a k-mer with its reverse complement into a single number.
```
./kmc -k27 -sc input.fastq 27mers .
```
`-sc` keeps the two directions apart: each canonical k-mer gets two counters, the number of times
it was read as itself and the number of times it was read as its reverse complement. `kmc_dump`
prints them as two columns:
```
./bin/kmc_dump 27mers 27mers.txt      # <k-mer> <forward count> <reverse count>
```
`-cb<value>` additionally requires each strand to be seen at least `<value>` times, which combines
with the usual cumulative `-ci`/`-cx` thresholds. For instance
```
./kmc -k27 -ci10 -cb1 input.fastq 27mers .
```
keeps k-mers seen at least 10 times in total *and* at least once on each strand.

`-mq<value>` drops every k-mer whose middle base (offset k/2 within the k-mer) has a Phred quality below
`<value>`, without affecting the neighbouring k-mers. It applies to FASTQ input only, and a read whose
qualities are all identical is left unfiltered because such qualities carry no information. It cannot be
combined with `-hc`.
```
./kmc -k21 -ci3 -cb1 -mq10 input.fastq 21mers .
```

Notes:
 * k-mers equal to their own reverse complement have no strand and are not reported.
 * `-sc` implies canonical k-mers, so it cannot be combined with `-b`; it is also not supported
   with strict memory mode (`-sm`) or KFF output.
 * The resulting database uses a distinct format version, so older readers reject it rather than
   misparsing the second counter. `kmc_tools` reads it in `info` and refuses the other operations.
