...
void usage()
{
	cout << "K-Mer Counter (KMC) ver. " << KMC::CfgConsts::kmc_ver << " (" << KMC::CfgConsts::kmc_date << ")\n"
		...
		<< "  -sc - store a separate counter per strand: for each canonical k-mer the number of times\n"
		<< "        it was read as itself and as its reverse complement (implies canonical k-mers)\n"
		<< "  -cb<value> - exclude k-mers whose weaker strand occurs less than <value> times; implies -sc\n"
		<< "  -mq<value> - exclude k-mers whose middle base (offset k/2) has Phred quality below <value>; FASTQ input only,\n"
		<< "        reads with uniform qualities are not filtered (default: 0, i.e. keep all)\n"
...
bool parse_parameters(int argc, char* argv[], Params& params)
{
	...
	for (i = 1; i < argc; ++i)
	{
		...
		// Minimum middle-base quality (must be tested before the -m memory limit prefix)
		else if (strncmp(argv[i], "-mq", 3) == 0)
			stage1Params.SetMinMiddleBaseQuality(atoi(&argv[i][3]));
		// Memory limit
		...
		// Minimum per-strand counter threshold (stranded counters)
		else if (strncmp(argv[i], "-cb", 3) == 0)
		{
			stage2Params.SetCutoffMinPerStrand(atoll(&argv[i][3]));
			stage1Params.SetStrandedCounters(true);
		}
		// Set p1
		...
		else if (strncmp(argv[i], "-sc", 3) == 0 && strlen(argv[i]) == 3)
			stage1Params.SetStrandedCounters(true);
