...
bool CSmallKCompleter::CompleteKMCFormat(CSmallKBuf<COUNTER_TYPE> result)
{
	...
	uint64 counters_bytes = counter_size * n_counters;
	...
	uint32 suf_recs;
	if (counters_bytes + kmer_suf_bytes == 0) //fixes #178
		suf_recs = 0;
	else
		suf_recs = (uint32)(suf_size / (counters_bytes + kmer_suf_bytes) * (counters_bytes + kmer_suf_bytes));
	uint32 suf_pos = 0;
	...
	for (kmer.data = 0; kmer.data < (1ull << 2 * kmer_len); ++kmer.data)
	{
		...
		//with stranded counters the array holds two slots per k-mer: [2*i] forward, [2*i+1] reverse
		uint64 fwd_idx = stranded_counters ? 2 * kmer.data : kmer.data;
		uint64 total = stranded_counters
			? (uint64)result.buf[fwd_idx] + (uint64)result.buf[fwd_idx + 1]
			: (uint64)result.buf[fwd_idx];

		//a k-mer that is its own reverse complement has no strand, so it is not reported
		if (total && stranded_counters && IsSelfRevComp(kmer, kmer_len))
			total = 0;

		if (total) //k-mer exists
		{
			++n_unique;

			uint64 weaker_strand = stranded_counters
				? MIN((uint64)result.buf[fwd_idx], (uint64)result.buf[fwd_idx + 1])
				: total;

			if (total < cutoff_min || weaker_strand < cutoff_min_strand)
				++n_cutoff_min;
			else if (total > (uint64)cutoff_max)
				++n_cutoff_max;
			else
			{
				++kmer_no;
				if (!this->without_output)
				{
					...
					for (uint32 c = 0; c < n_counters; ++c)
					{
						if (result.buf[fwd_idx + c] > (COUNTER_TYPE)counter_max)
							result.buf[fwd_idx + c] = (COUNTER_TYPE)counter_max;
						result.Store(fwd_idx + c, suf, suf_pos, counter_size);
					}

					if (suf_pos >= suf_recs * (kmer_suf_bytes + counters_bytes))
					{
	...
	if (!this->without_output)
	{
		...
		// counter mode: 0 - a single counter per k-mer, 1 - a pair of per-strand counters
		store_uint(pre_file, stranded_counters ? 1 : 0, 1);		offset++;
		store_uint(pre_file, 0, 1);			offset++;
		store_uint(pre_file, 0, 1);			offset++;

		store_uint(pre_file, cutoff_max >> 32, 4);				offset += 4;
		// minimal per-strand count required to report a k-mer (stranded counters only)
		store_uint(pre_file, cutoff_min_strand, 4);				offset += 4;
		...
		//version marker: 0x0 - KMC 1.x format, 0x100 - KMC 1.x layout with per-strand counters
		store_uint(pre_file, stranded_counters ? 0x100 : 0x0, 4);
		offset += 4;
