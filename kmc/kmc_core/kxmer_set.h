...
//Number of heap entries needed for max_x == 3: InitKXMerSet() adds 1 + 5 + 21 + 85 sub-ranges.
#define MAX_FOR_X_3 112

//With stranded counters the sub-ranges are built once per strand, so the heap has to hold twice
//as many entries. KXMER_SET_LAST_INNER is the deepest heap node that can still have children;
//nodes above it are leaves and the sift-down stops there.
#define KXMER_SET_MAX_ELEMS (2 * MAX_FOR_X_3)
#define KXMER_SET_LAST_INNER (KXMER_SET_MAX_ELEMS / 2)
static_assert(KXMER_SET_MAX_ELEMS + 2 <= KXMER_SET_SIZE, "the k+x-mer heap is too small");
...
struct ParentFinder<SIZE, PARENT, Range<(PARENT <= KXMER_SET_LAST_INNER)> >
{
	FORCE_INLINE static uint32 Execute(CKXmerSet<SIZE>& ptr, const CKmer<SIZE>& kmer)
	{
		//unused heap slots are filled with the maximal k-mer, so both children may always be read
...
class CKXmerMerger
{
	...
	uint32 cutoff_min_strand;
	uint64 strand_split;   //first position in buffer holding a reverse-strand (k+x)-mer
...
public:
	...
	{
		this->stranded_counters = stranded_counters;
		this->cutoff_min_strand = cutoff_min_strand;
		this->strand_split = strand_split;
	}
...
{	
	...
	uchar* out_buffer;	
	uint32 n_threads = 0;	
	bool stranded_counters = false;
	uint32 cutoff_min_strand = 0;
	uint64 strand_split = 0;
...
public:
	CKXmerSetMultiThreaded(CKmer<SIZE>* buffer,
		...
		out_buffer(out_buffer),		
		n_threads(n_threads),
		stranded_counters(stranded_counters),
		cutoff_min_strand(cutoff_min_strand),
	...
	void Process(bool without_output, OutputType output_type)
	{
		...
		uint32 rec_len = (kmer_len - lut_prefix_len) / 4 + counter_size * (stranded_counters ? 2 : 1);
