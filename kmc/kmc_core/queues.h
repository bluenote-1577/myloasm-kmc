...
public:
	...
	vector<pair<int32, int64>> get_sorted_req_sizes(uint32 max_x, const uint64 size_of_kmer_t, uint32 cutoff_min, int64 cutoff_max, int64 counter_max, uint32 lut_prefix_len, bool stranded_counters, uint32 n_counters)
	{
		lock_guard<mutex> lck(mtx);
		...
		for (auto& p : m)
		{
			...
			uint64 out_buffer_size = max_out_recs * (kmer_bytes + counter_size * n_counters);

			uint32 rec_len = calc_sort_rec_len(kxmer_symbols, stranded_counters);
