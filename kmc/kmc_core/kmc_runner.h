...
namespace KMC
{
	...
	class Stage1Params
	{
		...
		bool strandedCounters = false;
		uint32_t minMiddleBaseQuality = 0;
	...
	public:		
		...
		Stage1Params& SetStrandedCounters(bool strandedCounters);
		// Drop every k-mer whose middle base (offset k/2) has a Phred quality below the threshold; 0 keeps all
		// k-mers. Only FASTQ input carries qualities. A read whose qualities are all identical is not filtered,
		// since such qualities carry no information.
		Stage1Params& SetMinMiddleBaseQuality(uint32_t minMiddleBaseQuality);
		...
		bool GetStrandedCounters() const noexcept { return strandedCounters; }
		uint32_t GetMinMiddleBaseQuality() const noexcept { return minMiddleBaseQuality; }
	...
	public:	
		...
		Stage2Params& SetCutoffMinPerStrand(uint64_t cutoffMinPerStrand);
		...
		uint64_t GetCutoffMinPerStrand() const noexcept { return cutoffMinPerStrand; }
