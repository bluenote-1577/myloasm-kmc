...
private:
	...
	bool both_strands;	
	bool stranded_counters;
	uint32 strand_bit_pos;   // bit position of the strand flag inside a (k+x)-mer record
	uint64 strand_split;     // first position in `buffer` holding a reverse-strand record
	bool without_output;

	CSignatureMapper* s_mapper;

	uint64 n_unique, n_cutoff_min, n_cutoff_max, n_total;
	uint32 cutoff_min, cutoff_max, cutoff_min_strand;
	...
	void CompactKmers();
	void CompactKmersStranded();
	uint64 FindFirstReverseStrand(uint64 start_pos, uint64 end_pos);
	...
	sort_func(sort_func)
{
	...
	cutoff_min_strand = Params.cutoff_min_strand;
	counter_max = (uint32)Params.counter_max;
	max_x = Params.max_x;	
	stranded_counters = Params.stranded_counters;
	// the strand flag lives just above the (k+x)-mer, i.e. in the slot of symbol no. kxmer_symbols
	strand_bit_pos = 2 * (Params.kmer_len + (Params.max_x ? Params.max_x + 1 : 0));
	strand_split = 0;
...
template <unsigned SIZE> void CKmerBinSorter<SIZE>::ExpandKmersBoth(uint64 tmp_size)
{
	...
	while (pos < tmp_size)
	{
		...
		//`<=` so that a k-mer equal to its own reverse complement (a palindrome, possible for even k)
		//is attributed to the forward strand; the canonical value is the same either way
		bool kmer_lower = !(rev_kmer < kmer);
		kmer_can = kmer_lower ? kmer : rev_kmer;
		buffer_input[input_pos].set(kmer_can);
		if (stranded_counters && !kmer_lower)
			buffer_input[input_pos].set_2bits(1, strand_bit_pos);
		++input_pos;
...
template<unsigned SIZE> uint64 CKmerBinSorter<SIZE>::ExpandKxmerBothParallel(uint64 start_pos, uint64 end_pos, uint64 output_start, uint64 output_end)
{
	...
	// Closes the (k+x)-mer at `out_pos`: stores x and, for stranded counters, the strand of the
	// whole run (every k-mer packed into one (k+x)-mer has the same orientation by construction).
	auto finalize_kxmer = [&](uint64& out_pos, uint32 x_val, bool lower)
	{
		buffer_input[out_pos].set_2bits(x_val, kmer_len * 2 + max_x * 2);
		if (stranded_counters && !lower)
			buffer_input[out_pos].set_2bits(1, strand_bit_pos);
		++out_pos;
	};
	...
	while (pos < end_pos)
	{
		...
		//see ExpandKmersBoth(): palindromes count as forward
		kmer_lower = !(rev_kmer < kmer);
		x = 0;
		if (kmer_lower)
			...
			if (kmer_lower)
			{
				if (!(rev_kmer < kmer))
				{
					...
					if (x == max_x)
					{
						...
						finalize_kxmer(output_start, x, kmer_lower);
...
template <unsigned SIZE> void CKmerBinSorter<SIZE>::Sort()
{
	...
	rec_len = calc_sort_rec_len(kxmer_symbols, stranded_counters);
...
template <unsigned SIZE> void CKmerBinSorter<SIZE>::CompactKxmers()
{
	...
	if (n_plus_x_recs)
	{
		...
		//With stranded counters the strand flag is the most significant bit of a record, so the
		//reverse-strand (k+x)-mers form a suffix of the sorted array. Each of the two halves is
		//partitioned by x on its own; the heap then merges all of them by the extracted k-mer.
		strand_split = stranded_counters ? FindFirstReverseStrand(0, compacted_count) : compacted_count;

		uint64 group_bounds[3];
		uint32 n_groups;
		if (stranded_counters)
		{
			group_bounds[0] = 0;
			group_bounds[1] = strand_split;
			group_bounds[2] = compacted_count;
			n_groups = 2;
		}
		else
		{
			group_bounds[0] = 0;
			group_bounds[1] = compacted_count;
			n_groups = 1;
		}
		...
		{						
			CKXmerSetMultiThreaded<SIZE> kxmer_set_multithreaded(buffer, kxmer_counters, compacted_count, 
				...
				stranded_counters, cutoff_min_strand, strand_split);

			for (uint32 g = 0; g < n_groups; ++g)
			{
				...
				pos[0] = group_bounds[g];
				pos[4] = group_bounds[g + 1];
				for (uint32 i = 1; i < 4; ++i)
					pos[i] = FindFirstSymbOccur(pos[i - 1], pos[4], 0, i);
		...
		else
		{
			...
			next_kmer.clear();
			uint32 count = 0;
			uint32 count_fwd = 0, count_rev = 0;

			//accumulates one popped (k+x)-mer occurrence into the right per-strand counter
			auto add_occurrences = [&](uint64 pos)
			{
				if (pos < strand_split)
					count_fwd += kxmer_counters[pos];
				else
					count_rev += kxmer_counters[pos];
			};

			auto start_new_kmer = [&](uint64 pos)
			{
				count_fwd = count_rev = 0;
				count = kxmer_counters[pos];
				if (stranded_counters)
					add_occurrences(pos);
			};
			...
			auto store_kmer = [&]()
			{
				uint32 total = stranded_counters ? count_fwd + count_rev : count;
				n_total += total;
				//a k-mer that is its own reverse complement has no strand, so it is not reported
				if (stranded_counters && IsSelfRevComp(kmer, kmer_len))
					return;
				++n_unique;
				if (total < cutoff_min || (stranded_counters && MIN(count_fwd, count_rev) < cutoff_min_strand))
					n_cutoff_min++;
				else if (total > cutoff_max)
					n_cutoff_max++;
				else if (!without_output)
				{
					if (output_type == OutputType::KMC)
					{
						...
						if (stranded_counters)
						{
							uint32 cf = MIN(count_fwd, counter_max);
							uint32 cr = MIN(count_rev, counter_max);
							for (int32 j = 0; j < (int32)counter_size; ++j)
								out_buffer[out_pos++] = (cf >> (j * 8)) & 0xFF;
							for (int32 j = 0; j < (int32)counter_size; ++j)
								out_buffer[out_pos++] = (cr >> (j * 8)) & 0xFF;
						}
						else
						{
							uint32 c = MIN(total, counter_max);
							for (int32 j = 0; j < (int32)counter_size; ++j)
								out_buffer[out_pos++] = (c >> (j * 8)) & 0xFF;
						}
					}
...
//----------------------------------------------------------------------------------
//Binary search of the first record in [start_pos,end_pos) whose strand flag is set. Records are
//sorted with the strand flag as the most significant bit, so the reverse-strand records form a suffix.
template <unsigned SIZE> uint64 CKmerBinSorter<SIZE>::FindFirstReverseStrand(uint64 start_pos, uint64 end_pos)
{
	while (start_pos < end_pos)
	{
		uint64 middle_pos = (start_pos + end_pos) / 2;
		if (buffer[middle_pos].get_2bits(strand_bit_pos) == 0)
			start_pos = middle_pos + 1;
		else
			end_pos = middle_pos;
	}
...
//----------------------------------------------------------------------------------
//Variant of CompactKmers() (max_x == 0) producing a pair of per-strand counters.
//The forward-strand records occupy [0, split) and the reverse-strand ones [split, n_rec);
//both ranges are sorted by k-mer, so a two way merge yields the k-mers in order.
template <unsigned SIZE> void CKmerBinSorter<SIZE>::CompactKmersStranded()
{
	uint32 kmer_symbols = kmer_len - lut_prefix_len;
	uint64 kmer_bytes = kmer_symbols / 4;
	if (lut_prefix_len == 0)
	...
	if (n_rec)
	{
		const uint64 split = FindFirstReverseStrand(0, n_rec);
		uint64 fwd_pos = 0, rev_pos = split;

		while (fwd_pos < split || rev_pos < n_rec)
		{
			CKmer<SIZE> kmer, rev_cand;
			if (rev_pos < n_rec)
			{
				rev_cand = buffer[rev_pos];
				rev_cand.mask(kmer_mask);
			}

			if (fwd_pos >= split)
				kmer = rev_cand;
			else if (rev_pos >= n_rec || !(rev_cand < buffer[fwd_pos]))
				kmer = buffer[fwd_pos];
			else
				kmer = rev_cand;

			uint32 count_fwd = 0, count_rev = 0;
			while (fwd_pos < split && buffer[fwd_pos] == kmer)
			{
				++count_fwd;
				++fwd_pos;
			}
			...
			//a k-mer that is its own reverse complement has no strand, so it is not reported
			if (IsSelfRevComp(kmer, kmer_len))
				continue;

			++n_unique;
			uint32 count = count_fwd + count_rev;
			if (count < cutoff_min || MIN(count_fwd, count_rev) < cutoff_min_strand)
				++n_cutoff_min;
			else if (count > cutoff_max)
				++n_cutoff_max;
			else if (!without_output)
			{
				if (count_fwd > counter_max)
					count_fwd = counter_max;
				if (count_rev > counter_max)
					count_rev = counter_max;

				lut[kmer.remove_suffix(2 * kmer_symbols)]++;
				for (int32 j = (int32)kmer_bytes - 1; j >= 0; --j)
					out_buffer[out_pos++] = kmer.get_byte(j);
				for (int32 j = 0; j < (int32)counter_size; ++j)
					out_buffer[out_pos++] = (count_fwd >> (j * 8)) & 0xFF;
				for (int32 j = 0; j < (int32)counter_size; ++j)
					out_buffer[out_pos++] = (count_rev >> (j * 8)) & 0xFF;
			}
		}
	}
...
template <unsigned SIZE> void CKmerBinSorter<SIZE>::Compact()
{
	if (max_x)
		CompactKxmers();
	else if (stranded_counters)
		CompactKmersStranded();
	else
		CompactKmers();
}
