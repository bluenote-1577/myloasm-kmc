...
CSplitter::CSplitter(CKMCParams &Params, CKMCQueues &Queues)
{
	...
	if (Params.min_middle_base_quality)
		min_mid_qual_char = (uchar)(33 + Params.min_middle_base_quality);
	mid_offset = kmer_len / 2;
}

//----------------------------------------------------------------------------------
// Find the quality string of the FASTQ record whose sequence starts at part[seq_start].
// Returns nullptr when the record is not complete in the current part, or when all
// qualities are identical (they then carry no information, e.g. FASTA converted to FASTQ).
const uchar* CSplitter::LocateFastqQuality(uint64 seq_start) const
{
	uint64 p = seq_start;
	while (p < part_size && part[p] != '\n' && part[p] != '\r')
		++p;
	const uint64 seq_len = p - seq_start;
	if (seq_len == 0 || p >= part_size)
		return nullptr;

	// end of the sequence line, then the '+' line; a CR LF pair counts as one line end (same rule as GetSeq)
	++p;
	if (p < part_size && (part[p] == '\n' || part[p] == '\r') && part[p] != part[p - 1])
		++p;
	if (p >= part_size || part[p] != '+')
		return nullptr;
	while (p < part_size && part[p] != '\n' && part[p] != '\r')
	...
	if (p < part_size && (part[p] == '\n' || part[p] == '\r') && part[p] != part[p - 1])
		++p;
	if (p + seq_len > part_size)
		return nullptr;

	const uchar* qual = part + p;
	uint64 i = 1;
	while (i < seq_len && qual[i] == qual[0])
		++i;
	return i == seq_len ? nullptr : qual;
}
...
{	
	uint32 pos = 0;
	curr_qual = nullptr; // the reader does not deliver qualities of reads that do not fit in a part
...
bool CSplitter::GetSeq(char *seq, uint32 &seq_size, ReadType read_type)
{
	...
	else if (file_type == InputType::FASTQ)
	{
		...
		if (curr_read_len == 0)
		{
			...
			read_qual = min_mid_qual_char ? LocateFastqQuality(part_pos) : nullptr;
			curr_qual = read_qual;
		...
		{
			// this chunk restarts k-1 symbols before where the previous one ended
			if (read_qual)
				curr_qual = read_qual + (curr_read_len - (kmer_len - 1));
...
bool CSplitter::ProcessReads(uchar *_part, uint64 _part_size, ReadType read_type)
{
	...
	{		
		...
		//	n_reads++;
		const uchar* qual = curr_qual; // nullptr: no middle-base quality filtering for this sequence
		i = 0;
		len = 0;
		while (i + kmer_len - 1 < seq_size)
		{
			...
			for (; i < seq_size; ++i)
			{
				...
				// The k-mer ending at i (it is complete once len >= kmer_len - 1) has a low-quality middle base:
				// close the current super-k-mer in front of it and restart with the k-mer ending at i + 1, whose
				// first symbol is at i - kmer_len + 2. Unlike an 'N', the neighbouring k-mers are kept.
				if (qual && len >= kmer_len - 1 && qual[i - mid_offset] < min_mid_qual_char)
				{
					if (len >= kmer_len)
					{
						bin_no = s_mapper->get_bin_id(current_signature.get());
	...
	if (both_strands)
		while (GetSeq(seq, seq_size, read_type))
		{
			...
			for (; i < seq_size; ++i)
			{
				...
				if (qual && qual[i - mid_offset] < min_mid_qual_char) // low-quality middle base
					continue;

				// Find canonical kmer representation
				//`<=`: a k-mer equal to its own reverse complement is attributed to the forward strand
				bool kmer_lower = !(kmer_rev < kmer_str);
				kmer_can = kmer_lower ? kmer_str : kmer_rev;

				// with stranded counters the array holds two slots per k-mer: [2*kmer] forward, [2*kmer+1] reverse
				if (stranded_counters)
					++small_k_buf.buf[2 * kmer_can.data + (kmer_lower ? 0 : 1)];
				else
...
template <typename COUNTER_TYPE> void CWSmallKSplitter<COUNTER_TYPE>::operator()()
{
	...
	memset(small_k_buf.buf, 0, (1ull << 2 * kmer_len) * n_counters * sizeof(*small_k_buf.buf));
