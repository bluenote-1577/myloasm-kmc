...
//    * SIZE     - maximal size of the k-mer (divided by 32)
	...
	Stage1Params& Stage1Params::SetStrandedCounters(bool strandedCounters)
	{
		this->strandedCounters = strandedCounters;
		return *this;
	}
	Stage1Params& Stage1Params::SetMinMiddleBaseQuality(uint32_t minMiddleBaseQuality)
	{
		if (minMiddleBaseQuality > 93)
			throw std::runtime_error("Wrong parameter: minimum middle-base quality must be a Phred value in range <0,93>");
		this->minMiddleBaseQuality = minMiddleBaseQuality;
		return *this;
	}
	...
	Stage2Params& Stage2Params::SetCutoffMinPerStrand(uint64_t cutoffMinPerStrand)
	{
		this->cutoffMinPerStrand = cutoffMinPerStrand;
		return *this;
	}
	...
	public:
		Stage1Results RunStage1(const Stage1Params& stage1Params)
		{
			...
			_setmaxstdio(2040);
#endif			
			// Stranded counters keep one spare bit above the (k+x)-mer to record the strand,
			// so the record must be one bit wider than 2*k. Selecting the SIZE for k+1
			// guarantees that bit exists (2*k+1 <= 64*SIZE  <=>  k+1 <= 32*SIZE).
			uint32_t sizing_k = stage1Params.GetKmerLen() + (stage1Params.GetStrandedCounters() ? 1 : 0);
			if (sizing_k > MAX_K)
				throw std::runtime_error("stranded counters require k <= " + std::to_string(MAX_K - 1));
			app = std::make_unique<CApplication<KMER_WORDS>>(sizing_k);
