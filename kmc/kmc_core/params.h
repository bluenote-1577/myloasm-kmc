...
//TODO: reconsider !!!!
	...
	uint32 cutoff_min_strand;	// exclude k-mers whose weaker strand occurs less than times (stranded counters only)
	...
	bool stranded_counters;	// store separate counters for the forward and the reverse strand
	uint32 n_counters;		// number of counters stored per k-mer (1, or 2 for stranded counters)
	uint32 min_middle_base_quality;	// drop k-mers whose middle base has a Phred quality below this (0 = keep all; FASTQ only)
