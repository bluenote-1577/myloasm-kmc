...
template <unsigned SIZE> void CKMC<SIZE>::SetParamsStage1(const KMC::Stage1Params& stage1Params)
{
	...
	Params.stranded_counters = stage1Params.GetStrandedCounters();
	Params.n_counters = Params.stranded_counters ? 2 : 1;

	if (Params.stranded_counters && !Params.both_strands)
		throw std::runtime_error("stranded counters require canonical k-mers (they are the per-strand decomposition of a canonical count)");

	Params.min_middle_base_quality = stage1Params.GetMinMiddleBaseQuality();
	if (Params.min_middle_base_quality)
	{
		if (stage1Params.GetHomopolymerCompressed())
			throw std::runtime_error("minimum middle-base quality cannot be combined with homopolymer compression (bases and qualities would no longer line up)");
		if (Params.file_type == InputType::BAM)
			throw std::runtime_error("minimum middle-base quality is not implemented for BAM input");
		if (Params.file_type != InputType::FASTQ)
			stage1Params.GetWarningsLogger()->Log("minimum middle-base quality has no effect: only FASTQ input carries base qualities");
	}

	// A (k+x)-mer record occupies 2*(kmer_len + max_x + 1) bits of a CKmer<SIZE> (64*SIZE bits).
	// With stranded counters one more bit is needed on top of that to record the strand.
	{
		int32 spare_symbols = (int32)(32 * SIZE) - (int32)Params.kmer_len - (Params.stranded_counters ? 2 : 1);
		Params.max_x = (uint32)MAX(0, MIN((int32)KMER_X, spare_symbols));
	}
...
template <unsigned SIZE> void CKMC<SIZE>::SetParamsStage2(const KMC::Stage2Params& stage2Params)
{
	...
	Params.cutoff_min_strand = (uint32)stage2Params.GetCutoffMinPerStrand();

	if (Params.cutoff_min_strand && !Params.stranded_counters)
		throw std::runtime_error("per-strand cutoff requires stranded counters");

	if (Params.stranded_counters)
	{
		if (Params.counter_max == 1)
			throw std::runtime_error("stranded counters need counters to be stored, so counter_max must be greater than 1");
		if (Params.output_type != OutputType::KMC)
			throw std::runtime_error("stranded counters are supported only for the KMC output format");
		if (stage2Params.GetStrictMemoryMode())
			throw std::runtime_error("stranded counters are not supported in strict memory mode");
		// the weaker strand alone can never reach more than half of the total
		if (2ull * Params.cutoff_min_strand > (uint64)Params.cutoff_max)
			Params.warningsLogger->Log("per-strand cutoff cannot be satisfied together with cutoff_max, no k-mer will be reported");
	}
...
template <unsigned SIZE> bool CKMC<SIZE>::AdjustMemoryLimitsSmallK() 
{
	...
	int64 tmp_mem_part_small_k_buf = (1ll << 2 * Params.kmer_len) * Params.n_counters * sizeof(uint64_t);//no of possible k-mers * no of counters * counter size
...
KMC::Stage2Results CKMC<SIZE>::ProcessSmallKOptimization_Stage2()
{
	...
	const int64 n_slots = (1ll << 2 * Params.kmer_len) * Params.n_counters;

	for (int j = 1; j < Params.n_splitters; ++j)
	{
		for (int64 i = 0; i < n_slots; ++i)
	...
	if (Params.stranded_counters)
	{
		for (int64 j = 0; j < n_slots; j += 2)
			if (count_results[0].buf[j] + count_results[0].buf[j + 1]) ++n_kmers;
	}
	else
	{
		for (int64 j = 0; j < n_slots; ++j)
	...
	if (Params.output_type == OutputType::KMC)
	{
		...
		for (Params.lut_prefix_len = 1; Params.lut_prefix_len < 16; ++Params.lut_prefix_len)
		{
			...
			uint64 suf_mem = n_kmers * (suffix_len / 4 + counter_size * Params.n_counters);
...
template <unsigned SIZE> KMC::Stage2Results CKMC<SIZE>::ProcessStage2_impl()
{
	...
	auto sorted_bins = Queues.bd->get_sorted_req_sizes(Params.max_x, sizeof(CKmer<SIZE>), Params.cutoff_min, Params.cutoff_max, Params.counter_max, Params.lut_prefix_len, Params.stranded_counters, Params.n_counters);
