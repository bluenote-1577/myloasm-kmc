...
// Number of bytes of a (k+x)-mer record that take part in the radix sort. With stranded counters
// one extra bit above the (k+x)-mer holds the strand and must be sorted as well, so that records
// that differ only in strand are kept apart.
inline uint32 calc_sort_rec_len(uint32 kxmer_symbols, bool stranded_counters)
{
	if (stranded_counters)
		return (2 * kxmer_symbols + 2 + 7) / 8;
	return (kxmer_symbols + 3) / 4;
}
