...
void CKmerBinCompleter::ProcessBinsFirstStage()
{
	...
	while (!kq->empty())
	{
		...
		if (!without_output)
		{
			...
			{
				uint32_t rec_size = (kmer_len + 3) / 4 + counter_size * n_counters;
				for (auto& e : data_packs)									
...
void CKmerBinCompleter::ProcessBinsSecondStage()
{
	...
	if (!without_output)
	{
		if(output_type == OutputType::KMC)
		{
			...
			store_uint(out_lut, stranded_counters ? 1 : 0, 1);		offset++;
			// minimal per-strand count required to report a k-mer (stranded counters only)
			store_uint(out_lut, cutoff_min_strand, 4);				offset += 4;
			...
			// version marker: 0x200 - KMC2, 0x300 - KMC2 layout with per-strand counters.
			// A distinct marker makes readers that do not know about the second counter fail
			// cleanly instead of silently misparsing the suffix records.
			store_uint(out_lut, stranded_counters ? 0x300 : 0x200, 4);
			offset += 4;
