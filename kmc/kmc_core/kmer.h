...
{
	const uint32 n_bits = 2 * p;
	const uint32 n_words = n_bits >> 6;
	const uint32 r = n_bits & 63;

	//shifting a uint64 by 64 or more is undefined, so whole words are moved separately;
	//this happens when the k-mer occupies a whole number of words but the record is wider
	if (r)
	{
		for (uint32 i = 0; i < SIZE; ++i)
		{
			uint64 v = (i + n_words < SIZE) ? (data[i + n_words] >> r) : 0;
			if (i + n_words + 1 < SIZE)
				v += data[i + n_words + 1] << (64 - r);
			data[i] = v;
		}
	}
	else
	{
		for (uint32 i = 0; i < SIZE; ++i)
			data[i] = (i + n_words < SIZE) ? data[i + n_words] : 0;
	}
}
...
//A k-mer that equals its own reverse complement cannot be assigned to a strand: reading it from
//either strand yields the same sequence. Such k-mers (only possible for even k) are skipped when
//stranded counters are requested. The k-mer is expected right-aligned in the low 2*kmer_len bits.
template<unsigned SIZE> inline bool IsSelfRevComp(CKmer<SIZE>& kmer, uint32 kmer_len)
{
	if (kmer_len & 1)
		return false;
	for (uint32 i = 0, j = kmer_len - 1; i < j; ++i, --j)
		if (kmer.get_2bits(2 * i) + kmer.get_2bits(2 * j) != 3)
			return false;
	return true;
}
