...
class CSplitter
{
	...
	// Minimum middle-base quality filter (Params.min_middle_base_quality). `min_mid_qual_char` is the
	// threshold as a FASTQ quality character (0 when the filter is off). GetSeq() sets `curr_qual` to the
	// qualities that line up with the sequence it returned, or nullptr when there are none to filter on.
	uchar min_mid_qual_char = 0;
	uint32 mid_offset = 0;              // the k-mer ending at position i has its middle base at i - mid_offset
	const uchar* curr_qual = nullptr;
	const uchar* read_qual = nullptr;   // quality string of the FASTQ record currently being returned in chunks

	const uchar* LocateFastqQuality(uint64 seq_start) const;
