...
bool CKMCFile::ReadParamsFrom_prefix_file_buf(uint64 &size, open_mode _open_mode)
{
	...
	//supported versions: 0 = kmc1, 0x200 = kmc2, and their stranded-counter variants
	//0x100 / 0x300, whose suffix records carry a forward and a reverse counter
	...
	if (kmc_version == 0x200 || kmc_version == 0x300)
	{
		...
		if (kmc_version == 0x300)
		{
			uchar counter_mode = 0;
			result = fread(&counter_mode, 1, 1, file_pre);
			if (counter_mode != 1)
				return false;
			n_counters = 2;
			result = fread(&min_count_per_strand, 1, sizeof(uint32), file_pre);
		}
		...
		sufix_rec_size = sufix_size + counter_size * n_counters;	

		return true;
	}
	else if (kmc_version == 0 || kmc_version == 0x100)
	{
		my_fseek(file_pre, -8, SEEK_END);
		...
		uchar counter_mode = 0;
		result = fread(&counter_mode, 1, 1, file_pre);
		if (kmc_version == 0x100)
		{
			if (counter_mode != 1)
				return false;
			n_counters = 2;
		}
		my_fseek(file_pre, 2, SEEK_CUR);
...
// RET: true - if not EOF
//-------------------------------------------------------------------------------
// Read a single counter (counter_size bytes, LSB first) from the buffered suffix file.
//-------------------------------------------------------------------------------
uint64 CKMCFile::ReadCounterFromListingBuf()
{
	if (index_in_partial_buf == part_size)
		Reload_sufix_file_buf();

	uint64 value = sufix_file_buf[index_in_partial_buf++];
...
bool CKMCFile::ReadNextKmer(CKmerAPI &kmer, uint32 &count)
{
	...
	do
	{
		...
		{
			last_count_fwd = ReadCounterFromListingBuf();
			last_count_rev = (n_counters == 2) ? ReadCounterFromListingBuf() : 0;
			count = (uint32)(last_count_fwd + last_count_rev);
		}
		sufix_number++;
...
bool CKMCFile::ReadNextKmer(CKmerAPI &kmer, uint64 &count)
{
	...
	do
	{
		...
		else
		{
			last_count_fwd = ReadCounterFromListingBuf();
			last_count_rev = (n_counters == 2) ? ReadCounterFromListingBuf() : 0;
			count = last_count_fwd + last_count_rev;
		}
		sufix_number++;
...
//-------------------------------------------------------------------------------
// Return next kmer together with its per-strand counters. For a database without stranded
// counters the whole count is reported as the forward one.
//-------------------------------------------------------------------------------
bool CKMCFile::ReadNextKmer(CKmerAPI &kmer, uint64 &count_fwd, uint64 &count_rev)
{
	uint64 count;
	if (!ReadNextKmer(kmer, count))
		return false;
	count_fwd = last_count_fwd;
	count_rev = last_count_rev;
	return true;
}
			...
			if(is_opened == opened_for_RA)
			{
				...
				for(uint64 i = 0; i < total_kmers; i++)		
				{
					...
					else
					{
						...
						{
							uint32 partial = 0;
							for (uint32 b = 0; b < counter_size; b++)
							{
...
bool CKMCFile::Info(CKMCFileInfo& info)
{
	if (is_opened)
	{
		...
		info.n_counters = n_counters;
		info.min_count_per_strand = min_count_per_strand;
		return true;
	}
	return false;
}
	...
	if (found)
	{
		...
		else
		{
			...
			for (uint32 c = 0; c < n_counters; ++c)
			{
				...
				for (uint32 b = 0; b < counter_size; b++)
				{
					uint64 aux = 0x000000ff & *(sufix_byte_ptr + c * counter_size + b);
					aux = aux << 8 * b;
					partial = aux | partial;
				}
				if (c == 0)
					last_count_fwd = partial;
				else
					last_count_rev = partial;
				counter += partial;
			}
		}
		//applay filtering only if counter_size != 0
