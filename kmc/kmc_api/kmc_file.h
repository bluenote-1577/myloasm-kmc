...
struct CKMCFileInfo
{
	...
	bool both_strands;
	uint64 total_kmers;
	//number of counters stored per k-mer: 1 normally, 2 for a database built with stranded counters
	//(the forward counter first, then the reverse one)
	uint32 n_counters;
	//minimal per-strand count the database was built with (stranded counters only)
	uint32 min_count_per_strand;
};
...
protected:
	...
	uint32 n_counters;
	uint32 min_count_per_strand;
	uint64 last_count_fwd;   //per-strand counters of the k-mer returned last
	uint64 last_count_rev;
	...
	// Read a single counter from the buffered suffix file. Auxiliary function.
	uint64 ReadCounterFromListingBuf();
...
public:
	...
	bool IsKMC2() const noexcept { return kmc_version == 0x200 || kmc_version == 0x300; }

	// Return true if the database stores a separate counter per strand
	bool IsStranded() const noexcept { return n_counters == 2; }

	// Return next kmer in CKmerAPI &kmer. Return its counter in uint64 &count. Return true if not EOF
	// For a stranded database `count` is the sum of both per-strand counters.
	...
	// As above, but also returns the per-strand counters. For a database without stranded counters
	// the whole count is reported as the forward one and the reverse one is 0.
	bool ReadNextKmer(CKmerAPI &kmer, uint64 &count_fwd, uint64 &count_rev);
