...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		uint64 counter, counter_fwd, counter_rev;
		if (kmer_data_base.IsStranded())
		{
			//stranded database: print <k-mer> <forward count> <reverse count>
			while (kmer_data_base.ReadNextKmer(kmer_object, counter_fwd, counter_rev))
			{
				kmer_object.to_string(str);
				uint32 pos = _kmer_length;
				str[pos++] = '\t';
				pos += CNumericConversions::Int2PChar(counter_fwd, (uchar*)str + pos);
				str[pos++] = '\t';
				pos += CNumericConversions::Int2PChar(counter_rev, (uchar*)str + pos);
				str[pos++] = '\n';
				fwrite(str, 1, pos, out_file);
			}
		}
		else
		{
...
void print_info(void)
{
	std::cout << "KMC dump ver. " << KMC_VER << " (" << KMC_DATE << ")\n"
			  ...
			  << "\nFor a database built with stranded counters (kmc -sc) two counters are printed\n"
			  << "per k-mer: the number of times it was read as itself and as its reverse complement.\n";
}
